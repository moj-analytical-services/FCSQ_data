...
ca_cases <- child_act_csv %>% group_by(Year, Qtr, Type, Count_type, Public_private) %>% 
  summarise(Count = sum(Count)) %>% ungroup() %>% filter(Type == 'Cases') %>% mutate(Qtr = as.numeric(Qtr))
...
#Financial Remedy
fr_cases <- fr_csv %>% group_by(Year, Qtr, Type) %>% summarise(Count = sum(Count)) %>% 
  ungroup()
...
#Adoption
adopt_case <- adopt_csv %>% group_by(Year,Quarter,Type) %>% summarise(Count = sum(Count)) %>% 
  ungroup()
...
#Divorce
divorce_case <- divorce_csv %>% group_by(Year, Quarter, Type) %>% summarise(Count = sum(Count)) %>% 
  ungroup()
...
divorce_group <- divorce_csv %>% group_by(Year, Quarter, Order_type) %>% summarise(Count = sum(Count)) %>% ungroup()
...
dv_case <- dv_csv %>% group_by(Year, Quarter, Type) %>% summarise(Count = sum(Total)) %>% ungroup()
...
fmpo_case <- fmpo_csv %>% group_by(Year, Quarter, Type) %>% summarise(Count = sum(Total)) %>% ungroup()
...
#FGMPO
fgmpo_case <- fgm_csv %>% group_by(Year, Quarter, Type) %>% summarise(Count = sum(Total)) %>% ungroup()
...
all_cases <- table_1_prep %>% group_by(Year, Quarter, Stage) %>% summarise(Count = sum(Count)) %>% ungroup()
